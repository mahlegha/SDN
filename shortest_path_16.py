...
#        self.W = np.zeros(shape=(1,1,10))
        ...
        # self.A[7][8]=1
        # self.A[8][7]=1
        self.B = np.zeros(shape=(7,16))
        self.C = np.zeros(shape=(7,16))
        self.D = np.zeros(shape=(1,17))
        self.Z = np.zeros(shape=(1,7,16))
        self.W = np.zeros(shape=(1,1,17))
        
        self.tx_pkts = np.zeros(shape=(7,16))
        self.nn = 0
        self.G = nx.DiGraph() 
        self.datapaths = {}
        self.min_weight = 0
    ...
    def _packet_in_handler(self, ev):
        ...
        self.hosts2 = ['00:00:00:00:12:00','00:00:00:00:13:00','00:00:00:00:14:00','00:00:00:00:15:00',
                      '00:00:00:00:16:00'] 
        ...
        if dst not in self.G:
            ...
            if dpid != d_sw:
                ...
                sio.savemat('next_hop_16.mat', {'w':self.W})
                out_port = self.G[dpid][self.nn]['port']
                # print('out_port: {}'.format(out_port))
            else:
                self.D[0][16] = 1    #recieved to dst
                self.W = np.concatenate((self.W, self.D[None]), axis=0)
                sio.savemat('next_hop_16.mat', {'w':self.W})
            ...
            sio.savemat('matrix_state_16.mat', {'z':self.Z})
